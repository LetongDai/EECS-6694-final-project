...
def find_latest_checkpoint(checkpoint_dir="debug_checkpoints", pattern="*_final.pt", verbose=True):
    """
    自动查找最新的checkpoint文件
    
    Args:
        checkpoint_dir: checkpoint目录路径
        pattern: 文件匹配模式，默认查找 *_final.pt
        verbose: 是否显示详细信息
    
    Returns:
        最新checkpoint文件的路径字符串，如果未找到则返回None
    """
    ...
    if not checkpoints:
        # 如果没有找到final文件，尝试找所有.pt文件
        if verbose:
            print(f"⚠️  未找到 {pattern} 文件，尝试查找所有 .pt 文件")
        checkpoints = list(checkpoint_path.glob("*.pt"))
    
    if not checkpoints:
        if verbose:
            print(f"❌ 在 {checkpoint_dir} 中未找到任何 .pt 文件")
        return None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # 自动查找最新的checkpoint
        print("=" * 70)
        print("🔍 自动查找最新checkpoint文件...")
        print("=" * 70)
        checkpoint_path = find_latest_checkpoint(verbose=True)
        
        if checkpoint_path is None:
            print("\n❌ 未找到checkpoint文件")
            print("   请确保 debug_checkpoints 目录存在且包含 .pt 文件")
            print("   或者使用命令行参数指定checkpoint路径:")
            print("   python analyse.py <checkpoint_path>")
            sys.exit(1)
