...
class MicrogridEnv:
    ...
    def _run_auction(self, env_idx: int, actions: np.ndarray, env_data: Dict) -> Dict:
        ...
        grid_sell_price, grid_buy_price = self._get_grid_price(time_hour)
        ...
        # Add Main Grid as last supplier (infinite capacity)
        suppliers.append({
            'name': 'main_grid',
            'power': 999.0,
            'bid': grid_buy_price,  # Main Grid's selling price to MG
            'cost': 0.0
        })
        ...
        # Calculate total demand (including battery charging)
        battery_charge_demand = 0
        if battery_mode == 'charge':
            battery_charge_demand = battery_power

        total_demand_with_battery = actual_demand + battery_charge_demand

        # Market clearing
        if total_demand_with_battery <= 0:
            ...
            for supplier in suppliers:
                if cumulative >= total_demand_with_battery:
                    allocated_power[supplier['name']] = 0
                elif cumulative + supplier['power'] <= total_demand_with_battery:
                    # Fully allocated
                    ...
                    allocated_power[supplier['name']] = total_demand_with_battery - cumulative
                    cumulative = total_demand_with_battery
                    clearing_price = supplier['bid']
                    break

        # Handle unsold energy (sellers can sell to Main Grid at buy rate)
        grid_purchases = 0  # Main Grid buying from MG
        for supplier in suppliers:
            if supplier['name'] in ['wind', 'solar', 'diesel']:
                accepted = allocated_power.get(supplier['name'], 0)
                unsold = supplier['power'] - accepted
                if unsold > 0:
                    # Sell to Main Grid at grid_sell_price (MG's selling price)
                    grid_purchases += unsold
                    allocated_power[supplier['name']] = supplier['power']  # Total including grid sale
        ...
        main_grid_sales = allocated_power.get('main_grid', 0)  # MG buying from grid
        main_grid_net = main_grid_sales - grid_purchases  # Positive = net import
    ...
    def _calculate_rewards(self, env_idx: int, auction_results: Dict, env_data: Dict, 
        ...
        RENEWABLE_SUBSIDY = 3      # 新能源补贴 (cents/kWh)
                                    # 建议范围: 1.5-3.0
                                    # 越高越鼓励新能源
        
        CARBON_TAX = 2.5             # 碳税/污染税 (cents/kWh)
                                    # 建议范围: 1.0-2.5
                                    # 越高越抑制传统能源
