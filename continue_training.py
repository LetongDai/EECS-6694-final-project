...
class ContinueTrainingConfig:
    ...
    def __init__(self, checkpoint_path):
        ...
        self.actor_lr = getattr(self, 'actor_lr', 1e-4) * 0.5  # 降低50%
        self.critic_lr = getattr(self, 'critic_lr', 1e-3) * 0.5
...
class ContinueTrainer:
    ...
    def __init__(self, checkpoint_path, config=None):
        ...
        for i, agent_name in enumerate(self.env.agent_names):
            ...
            agent.critic.load_state_dict(self.checkpoint['agents'][i])  # 如果checkpoint里有
            agent.target_critic.load_state_dict(self.checkpoint['agents'][i])
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        checkpoint_path = "final_models/maddpg_microgrid_best_20251124_105816/model_checkpoint.pt"
    ...
    if not Path(checkpoint_path).exists():
        ...
        print(f"\n请指定正确的checkpoint路径:")
        print(f"   python continue_training.py <checkpoint_path>")
        return
